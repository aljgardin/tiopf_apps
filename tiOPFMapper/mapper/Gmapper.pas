...
type
  ...
  {AJG: Added:}
  TMGInclude = class;
  TMGIncludeList = class;
  TMGReference = class;
  TMGReferenceList = class;
  {AJG: End.}
  ...
  {AJG: Added:}
  private
    FIncludeFiles: TMGIncludeList;
    function GetIncludeFiles: TMGIncludeList;
    procedure SetIncludeFiles(AValue: TMGIncludeList);
  protected
  ...
  published
    //  IncludeFiles: TMGIncludeList, a Mediator friendly object.
    property IncludeFiles: TMGIncludeList read GetIncludeFiles write SetIncludeFiles;
  {AJG: End.}
...
//    property FieldLength: Integer read FFieldLength write SetFieldLength;
  ...
  {AJG: Added:}
  private
//    FDirtyOnPropertyChange: boolean;
//    procedure SetDirtyOnPropertyChange(AValue: boolean);
  public
    procedure AssignClassProps(ASource: TtiObject); override;
  published
    // On write class:Property, add "Dirty := true;" in Property Setter
//    property DirtyOnPropertyChange: boolean read FDirtyOnPropertyChange write SetDirtyOnPropertyChange;
  {AJG: End.}
  ...
  {AJG: Added:}
  private
    FUnitReferences: TMGReferenceList;
    function GetUnitReferences: TMGReferenceList;
    procedure SetUnitReferences(AValue: TMGReferenceList);
  protected
    // When Does the References: TStringlist Change?
    procedure OnReferencesChange(AObject: TObject);
  public
    // A mediator friendly References:
    property UnitReferences: TMGReferenceList read GetUnitReferences write SetUnitReferences;
    procedure AssignClassProps(ASource: TtiObject); override;
  {AJG: End.}
  ...
  {AJG: Added:}
  { TMGInclude: A mediator friendly object for an Include File Name. }
  TMGInclude = class(TBaseMapObject)
  protected
  ...
  {AJG: Added:}
  { TMGIncludeList }
  TMGIncludeList = class(TBaseMapObjectList)
  protected
    procedure SetItems(i: integer; const AValue: TMGInclude); reintroduce;
    function GetItems(i: integer): TMGInclude; reintroduce;
  public
    property Items[i: integer]: TMGInclude read GetItems write SetItems;
    procedure Add(AObject: TMGInclude); reintroduce;
    function FindByName(Const AName: string): integer;
  end;
  {AJG: End.}

  {AJG: Added:}
  { TMGReference:  A mediator friendly object for a Unit Reference.}
  TMGReference = class(TBaseMapObject)
  protected
    FReference: string;
    FUnitOID: string;
  ...
  {AJG: Added:}
  { TMGReferenceList }
  TMGReferenceList = class(TBaseMapObjectList)
  protected
    procedure SetItems(i: integer; const AValue: TMGReference); reintroduce;
    function GetItems(i: integer): TMGReference; reintroduce;
  public
    property Items[i: integer]: TMGReference read GetItems write SetItems;
    procedure Add(AObject: TMGReference); reintroduce;
    function FindByName(Const AName: string): integer;
  end;
  {AJG: End.}
...
function gClassDefTypeToStr(const AClassDefType: TClassDefType): String;
begin
  result := typinfo.GetEnumName(TypeInfo(TClassDefType), Integer(AClassDefType));
end;

function gOIDTypeToStr(const AOIDType: TOIDType): String;
begin
  result := typinfo.GetEnumName(TypeInfo(TOIDType), Integer(AOIDType));
end;
...
begin
  ...
  {AJG: Added:}
  FIncludeFiles := TMGIncludeList.Create;
...
begin
  ...
  {AJG: Added:}
  FIncludeFiles.Destroy;
  Includes.OnChange := nil;
  {AJG: End.}
...
{AJG: Added:}
procedure TMapProject.SetIncludeFiles(AValue: TMGIncludeList);
var
  ic: Integer;
begin
  ...
  //Copy IncludeFiles To TMapProject.Inlcudes ancestor.
  Includes.Clear;
  Includes.BeginUpdate;
  for ic := 0 to FIncludeFiles.Count - 1 do
    Includes.Add(FIncludeFiles.Items[ic].IncludeFilename);
  Includes.EndUpdate;
end;
{AJG: End.}

{AJG: Added:}
function TMapProject.GetIncludeFiles: TMGIncludeList;
begin
...
{AJG: Added:}
procedure TMapProject.OnIncludesChange(AObject: TObject);
var
  ic: Integer;
  temp: TMGInclude;
begin
  //Copy Includes to IncludeFIles.
  FIncludeFiles.BeginUpdate;
  FIncludeFiles.Clear;
  for ic := 0 to Includes.Count - 1 do
    begin
      temp := TMGInclude.Create;
...
begin
  ...
  FProjectClasses.Assign(TMapProject(ASource).ProjectClasses);
  FProjectEnums.Clear;
  FProjectEnums.Assign(TMapProject(ASource).ProjectEnums);

  //Published: Includes.Assign(TgMapProject(ASource).Includes);
  FIncludeFiles.Clear;
  FIncludeFiles.Assign(TMapProject(ASource).IncludeFiles);
end;
{AJG: End.}
...
begin
  ...
  {AJG: Added:}
  FUnitReferences := TMGReferenceList.Create;
...
begin
  ...
  {AJG: Added:}
  FUnitReferences.Destroy;
  references.OnChange := nil;
  {AJG: End.}
...
{AJG: Added:}
procedure TMapUnitDef.SetUnitReferences(AValue: TMGReferenceList);
var
  ic: Integer;
begin
  ...
  for ic := 0 to FUnitReferences.Count - 1 do
    References.Add(FUnitReferences.Items[ic].UnitName);
  References.EndUpdate;
end;
{AJG: End.}

{AJG: Added:}
procedure TMapUnitDef.OnReferencesChange(AObject: TObject);
var
  ic: integer;
  temp: TMGReference;
begin
  //Copy References to UnitReferences.
  FUnitReferences.BeginUpdate;
  FUnitReferences.Clear;
  for ic := 0 to References.Count - 1 do
    begin
      temp := TMGReference.Create;
      temp.Reference := References.Strings[ic];
      FUnitReferences.Add(temp);
    end;
  FUnitReferences.EndUpdate;
end;
{AJG: End.}
...
begin
  ...
  UnitClasses.Clear;
  UnitClasses.Assign(TMapUnitDef(ASource).UnitClasses);
  UnitEnums.Clear;
  UnitEnums.Assign(TMapUnitDef(ASource).UnitEnums);
  References.Clear;
  References.Assign(TMapUnitDef(ASource).References);

  UnitReferences.Clear;
  UnitReferences.Assign(TMapUnitDef(ASource).UnitReferences);
end;
{AJG: End.}

{AJG: Added:}
function TMapUnitDef.GetUnitReferences: TMGReferenceList;
begin
  result := FUnitReferences;
end;
{AJG: End.}
...
{AJG: Added:}
//procedure TMapClassDef.SetDirtyOnPropertyChange(AValue: boolean);
//begin
//  if FDirtyOnPropertyChange = AValue then Exit;
//  FDirtyOnPropertyChange := AValue;
//end;
{AJG: End.}
...
begin
  ...
  FClassMapping.Assign(TMapClassDef(ASource).ClassMapping);
  FSelections.Clear;
  FSelections.Assign(TMapClassDef(ASource).Selections);
  FValidators.Clear;
...
//procedure TPropMapping.SetFieldLength(AValue: Integer);
//begin
  ...
  for lCtr := 0 to lList.Params.Count - 1 do
    begin
      ...
      case lParam.ParamType of
        ...
        ptStream: Query.AssignParamFromStream(lParam.SQLParamName, TStream(lParam.Value));
      end;
    end;
*)  
end;
...
{AJG: Added:}
{TMGInclude}
procedure TMGInclude.SetIncludeFilename(const AValue: string);
begin
...
{AJG: Added:}
{TMGIncludeList }
procedure TMGIncludeList.Add(AObject: TMGInclude);
begin
  inherited Add(AObject);
end;

function TMGIncludeList.GetItems(i: integer): TMGInclude;
begin
  Result := inherited GetItems(i) as TMGInclude;
end;

procedure TMGIncludeList.SetItems(i: integer; const AValue: TMGInclude);
begin
  inherited SetItems(i, AValue);
end;

function TMGIncludeList.FindByName(const AName: string): integer;
var
  ic: integer;
begin
  result := 0;
  ...
  begin
    if LowerCase(Items[ic].IncludeFilename) = LowerCase(AName) then
    begin
      result := ic;
      break;
    end;
...
{AJG: Added:}
{TMGReference}
procedure TMGReference.SetReference(const AValue: string);
begin
  if FReference <> AValue then
    FReference := AValue;
end;
{AJG: End.}
...
{AJG: Added:}
{TMGReferenceList }
procedure TMGReferenceList.Add(AObject: TMGReference);
begin
  inherited Add(AObject);
end;

function TMGReferenceList.FindByName(const AName: string): integer;
var
  ic: integer;
begin
  result := 0;
  for ic := 0 to Count -1 do
  begin
    if LowerCase(Items[ic].Reference) = LowerCase(AName) then
    begin
      result := ic;
      break;
    end;
...
function TMGReferenceList.GetItems(i: integer): TMGReference;
begin
  Result := inherited GetItems(i) as TMGReference;
end;

procedure TMGReferenceList.SetItems(i: integer; const AValue: TMGReference);
begin
  inherited SetItems(i, AValue);
end;
{AJG: End.}
